...
export default function HomePage() {
  ...
  return (
    <div className="min-h-dvh relative overflow-hidden">
      ...
      {/* 위로 올라가는 검은 배경 */}
      <div
        className={`absolute inset-0 bg-black transition-transform ${isDarkMode ? 'duration-[4000ms] ease-out' : 'duration-[1000ms] ease-in'} ${
      ...
      <div className="relative z-10 flex flex-col justify-center items-center mt-40">
        <div className="text-center animate-diagonal-gradient">
          <p className="text-[44px] font-extrabold text-black mb-2">
            숨은 위험까지 찾아내는 든든한 계약 비서,
          </p>
          <h1
            className="text-[120px] font-black text-black animate-diagonal-gradient"
            style={{
              fontWeight: 1000,
              letterSpacing: '2px',
        ...
        <h2 className="font-bold text-xl text-secondary mt-20">
          지금 바로 사용해보세요!
        </h2>
        ...
        <img src={userSvg} className="w-300 h-auto" />
        ...
        <h1 className="font-black text-3xl text-[#121212] mb-[150px] whitespace-pre-line text-center mt-6 animate-bounce-slow-delayed">
          AI 개인 법률 리스크 관리 서비스 Checky로 해결.
        </h1>
        <div
          ref={featuresRef}
          className={`mt-[50px] w-full flex items-center transition-all duration-700 ease-out ${
        ...
        >
          <div className="basis-1/2 flex flex-col items-start space-y-2 pl-[180px]">
            <span
              className={`inline-flex items-center rounded-full bg-[#D8D8D8]/[0.72] text-secondary
                            px-3 py-1 text-xl font-bold mb-6 transition-all duration-600 ease-out delay-100 ${
                              visibleSections.has('features')
            ...
            <div
              className={`font-bold text-4xl text-[#121212] mb-3 transition-all duration-600 ease-out delay-200 ${
                visibleSections.has('features')
                  ? 'transform translate-x-0 opacity-100'
            ...
            <h1
              className={`text-xl text-[#4A5344] leading-relaxed break-keep whitespace-pre-line transition-all duration-600 ease-out delay-300 ${
                visibleSections.has('features')
                  ? 'transform translate-x-0 opacity-100'
                  : 'transform -translate-x-6 opacity-0'
              }`}
          ...
          <div className="basis-1/2 flex justify-end">
            <img
              src={reportSvg}
              className={`w-[280px] sm:w-[480px] md:w-[600px] h-auto mt-20 transition-all duration-800 ease-out delay-400 ${
                visibleSections.has('features')
        ...
        <div
          ...
          className={`mt-[100px] w-full flex items-center flex-row-reverse transition-all duration-700 ease-out ${
            visibleSections.has('second')
              ? 'transform translate-y-0 opacity-100'
              : 'transform translate-y-12 opacity-0'
          }`}
        >
          ...
          <div className="basis-1/2 flex justify-start">
            <img
              src={safeSvg}
              className={`w-[280px] sm:w-[540px] md:w-[680px] h-auto mt-20 transition-all duration-800 ease-out delay-400 ${
                visibleSections.has('second')
                  ? 'transform translate-y-0 scale-100 opacity-100'
                  : 'transform translate-y-8 scale-95 opacity-0'
              }`}
        ...
        <div
          ref={lastSectionRef}
          className="mt-[80px] w-full flex items-center flex-row-reverse relative z-20"
        >
          <div className="basis-1/2 flex flex-col justify-center items-center space-y-2 pr-32 text-center">
            <div
              className={`font-bold text-4xl text-[#FFFFFF] mt-[150px] mb-3 whitespace-pre-line transition-all duration-1000 ease-out ${
                isDarkMode
                  ? 'transform translate-y-0 opacity-100'
                  : 'transform translate-y-8 opacity-90'
              }`}
            >
              {'안심되는 계약서 작성,\n체키와 지금 시작해보세요'}
            </div>
            <h1
              className={`text-md text-[#FFFFFF] leading-relaxed break-keep whitespace-pre-line transition-all duration-1000 ease-out delay-200 ${
                isDarkMode
                  ? 'transform translate-y-0 opacity-100'
                  : 'transform translate-y-8 opacity-90'
              }`}
            >
              {'개인정보 보호를 위해 모든 정보는 24시간 내 영구 삭제됩니다.'}
            </h1>
            <button
              onClick={() => (window.location.href = '/upload')}
              className={`mt-[30px] mb-[180px] transition-all duration-3000 ease-out delay-400 hover:scale-105 bg-white text-green px-6 py-2 rounded-[10px] font-bold hover:bg-gray-100 ${
                isDarkMode
                  ? 'transform translate-y-0 opacity-100'
                  : 'transform translate-y-8 opacity-90'
              }`}
          ...
          <div className="basis-1/2 flex justify-start pl-35">
            <img
              src={macbookSvg}
              className={`w-[400px] sm:w-[720px] md:w-[960px] lg:w-[1200px] h-auto mt-20 transition-all duration-1000 ease-out delay-300 animate-bounce-float ${
                isDarkMode
                  ? 'transform translate-y-0 opacity-100'
                  : 'transform translate-y-8 opacity-90'
              }`}
